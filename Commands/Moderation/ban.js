...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addNumberOption((option) => option
      .setName('days')
      .setDescription(`How long the user will stay banned.`)
    )
  ...
  async execute(interaction) {
    ...
    if (!days) {
      ...
      BanInfo.author = author;
      let msg = `${author} has permanently banned you from ${interaction.guild.name}. Duration: ${days} days. If you wish to appeal this decision, please do so at https://discord.gg/zeKqgufjvb`;
      if (reason) {
        banEmbed.addField('Reason', reason);
        msg += ` Reason: ${reason}`;
        BanInfo.reason = reason;
